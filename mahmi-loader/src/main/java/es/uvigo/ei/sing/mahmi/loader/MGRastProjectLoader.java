...
/**
 * {@linkplain MGRastProjectLoader} is an implementation for
 * {@link ProjectLoader} to loading MGRast based projects. Is the default
 * project loader of MAHMI
 * 
...
public final class MGRastProjectLoader implements ProjectLoader {
	...
	// private static final FastaReader<AminoAcidSequence> proteinReader =
	// FastaReader.forAminoAcid();

	// TODO: receive in constructor
	private final PSortFastaFilter psort = PSortFastaFilter.of(Positive, Extracellular.single());
	...
	/**
	 * Loads a MAHMI project (metagenomes and proteomes) from a folder and
	 ...
	 * 
	 * @param reader
	 *            The {@link FastaReader}
	 * @param file
	 *            The {@linkplain Path} of the FASTA file
	 * @return The read {@link Fasta}
	 */
	private <A extends CompoundSequence<?>> Fasta<A> readFasta(	final FastaReader<A> reader,
																final Path file) {
		try {
	 ...
	 */
	private Stream<Path> findFiles(final Path directory, final PathMatcher filePattern) {
		try {
			return Files.walk(directory).filter(path -> filePattern.matches(path.getFileName()))
