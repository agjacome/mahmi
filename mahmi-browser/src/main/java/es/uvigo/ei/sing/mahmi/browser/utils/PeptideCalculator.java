...
public class PeptideCalculator {
	...
	final static Double[] cterm    = { 2.35, 2.17, 2.02, 2.09, 1.71, 2.19, 2.17, 2.34, 1.82, 2.36, 2.36, 
	...
	private double doFormula(final double pI, final String sequence){
		...
		result = result-1/(1+(Math.pow(10,(cterm[findElement(sequence.charAt(sequence.length()-1))]-pI))));		
		for (int i = 0; i < sequence.length(); i++){
